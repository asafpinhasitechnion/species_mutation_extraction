...
def compute_mutation_density(mutation_file, chrom, chrom_length, bin_size=1000, slide=1000,mut_regex=None):
    df = pd.read_csv(mutation_file, compression='gzip')
...
def main():
    ...
    interval_files = [os.path.join(args.coverage_dir, f) for f in os.listdir(args.coverage_dir)] if args.coverage_dir else []
    interval_files.sort()

    mutation_regex = None
    if args.mutation_category:
        mutation_regex = re.compile(args.mutation_category)

    midpoints_list = []
    values_list = []

    for i, mutation_file in enumerate(mutation_files):
        midpoints, mutations = compute_mutation_density(mutation_file, chrom, chrom_length, args.bin_size, args.slide, mutation_regex)
        if args.coverage_dir:
            coverage_file = interval_files[i]
            coverage = compute_binned_coverage(coverage_file, chrom, chrom_length, args.bin_size, args.slide, mutation_regex)
